...
export const contestsApi = {
  async create(input: CreateContestInput) {
    ...
    if (codesError) {
      // Rollback contest if codes fail? Or handle gracefully?
      console.error('Failed to generate participant codes:', codesError);
      // Consider: Should we delete the contest or allow admin to regenerate codes later?
    }
...
6. Verify:
   ...
   - Database shows 50 participant codes generated
...
- [x] Implement API layer (contestsApi)
  - [x] create(): Insert contest + generate 50 participant codes
