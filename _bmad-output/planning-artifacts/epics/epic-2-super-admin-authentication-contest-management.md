...
**And** `participants` table exists with: id, contest_id, code (8-digit), status (unused/used), name, organization_name, tlc_name, tlc_email, created_at
**And** RLS policies restrict access to authenticated admins only
**Note:** Participant codes are NOT auto-generated at contest creation. Codes are generated on-demand via "Generate Codes" button (see Story 2.6). A contest can exist with 0 participant codes.

**Requirements:** FR6, FR19, ARCH7-8
...
**Requirements:** NFR1 (Performance), NFR2 (Stability)
